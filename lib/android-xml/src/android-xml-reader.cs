...
namespace AndroidXml
{
    public class AndroidXmlReader : XmlReader
    {
        ...
        private AndroidResources? _resources;
        ...
        public AndroidXmlReader(Stream source, AndroidResources resources)
            : this(new ResXMLParser(source), resources)
        {
        }
        ...
        public AndroidXmlReader(ResXMLParser parser, AndroidResources resources)
        {
            _parser = parser ?? throw new ArgumentNullException(nameof(parser));
            _namespaces = new Dictionary<string, List<string>>();
            _nameTable = new NameTable();
        ...
        {
            if (_parser.EventCode != ResXMLParser.XmlParserEventCode.START_TAG) return "";
        ...
        public override string GetAttribute(int i)
        {
            ...
            Console.WriteLine(attr.Name + "--" + attr.Namespace);
        ...
        public override bool MoveToNextAttribute()
        {
            ...
            return MoveToAttribute(nextIndex);
        }
                              ...
                              string prefix = "", string localName = "", string namespaceUri = "", string value = "")
        {
            ...
            string? resVal;
            if ((resVal = MaskValues.Instance.IsListed(localName, value)) != null)
            {
                _value = resVal;
                return;
            }

            var attrValue = AttrsJsonValues.Instance.GetValue(localName, value);

            if ((_resources is not null) && (value.StartsWith('@')))
            {
                string? getRes;
                if ((getRes = _resources.Get(value)) is not null)
                {
                    _value = "@" + getRes;
                }
                else
            ...
            // Console.WriteLine(prefix + "--" + localName + "--" + namespaceUri + "--" + value);
